...
def format_console(review, show_confidence=False):
    ...
    if issues:
        console.print(Panel(f"Found {len(issues)} issues to review", title="🔍 Issues Found", style="bold yellow"))
        ...
        for i, issue in enumerate(issues, 1):
            ...
            if issue.get('start_line') and issue.get('end_line'):
                issue_text += f"Lines: [dim]{issue.get('start_line')}-{issue.get('end_line')}[/dim]\n"
    ...
    if review.get('technical_analysis'):
        ...
        if tech_text:
            console.print(Panel(tech_text, title="🔧 Technical Analysis", box=ROUNDED, style="bold cyan"))
    ...
    if review.get('architectural_consistency'):
        ...
        if arch_text:
            console.print(Panel(arch_text, title="🏗️ Architectural Consistency", box=ROUNDED, style="bold magenta"))
    
    # Print legacy fields for backward compatibility
    if review.get('security_concerns') and review['security_concerns'] != 'N/A':
        console.print(Panel(review['security_concerns'], title="🔒 Security Concerns", box=ROUNDED, style="bold red"))
    
    if review.get('performance_concerns') and review['performance_concerns'] != 'N/A':
        console.print(Panel(review['performance_concerns'], title="⚡ Performance Concerns", box=ROUNDED, style="bold yellow"))
    
    if review.get('style_concerns') and review['style_concerns'] != 'N/A':
        console.print(Panel(review['style_concerns'], title="🎨 Style Concerns", box=ROUNDED, style="bold blue"))
    
    if review.get('suggestions_for_improvement'):
        console.print(Panel(review['suggestions_for_improvement'], title="💡 Suggestions for Improvement", box=ROUNDED, style="bold green"))
    ...
    if review.get('overall_assessment'):
        ...
        if isinstance(assessment, dict):
            ...
            if assessment_text:
                console.print(Panel(assessment_text, title="📊 Overall Assessment", box=ROUNDED, style="bold"))
        else:
            console.print(Panel(str(assessment), title="📊 Overall Assessment", box=ROUNDED, style="bold"))
    
    # Print confidence if available
    confidence = calculate_confidence(review)
    if show_confidence and confidence:
        console.print(Panel(f"Confidence: {confidence}", title="🎯 Confidence", box=ROUNDED, style="bold"))
